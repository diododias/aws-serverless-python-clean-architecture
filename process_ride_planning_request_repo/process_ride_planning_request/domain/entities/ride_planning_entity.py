...
from process_ride_planning_request.domain.entities.address_entity import AddressEntity
from process_ride_planning_request.domain.entities.ride_option_entity import RideOptionEntity
from process_ride_planning_request.domain.entities.ride_planning_status_enum import RidePlanningStatusEnum
from process_ride_planning_request.domain.exceptions.invalid_state_exception import InvalidStateException
...
class RidePlanningEntity:
    ...
    def add_ride_option(self, ride_option: RideOptionEntity) -> None:
        if ride_option in self.ride_options:
            return
        if self.status == RidePlanningStatusEnum.WAITING_FOR_APPROVAL:
            raise InvalidStateException("Cannot add ride option to Ride Planning in Waiting For Approval State")
        self.ride_options.append(ride_option)

    def set_waiting_for_approval_status(self) -> None:
        if len(self.ride_options) == 0:
            raise InvalidStateException("Must add a one ride option to set Waiting for Approval State")
