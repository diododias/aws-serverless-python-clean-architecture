...
from src.drivers_adapters.constants import PRIMARY_KEY, SORT_KEY, ONE_YEAR_IN_DAYS
...
def map_ride_planning_to_persistence_schema(
    ...
    ride_planning_item["created_at"] = ride_planning.created_at.isoformat()
    ride_planning_item["modified_at"] = datetime.now().isoformat()
    ride_planning_item["time_to_live"] = Decimal((datetime.now() + timedelta(days=ONE_YEAR_IN_DAYS)).timestamp())
...
def map_persistence_schema_to_ride_planning(ride_planning_dto: Dict[str, str | Decimal | int]) -> RidePlanningEntity:
    ...
    return RidePlanningEntity(
        ride_planning_dto.get(SORT_KEY),
        ride_planning_dto.get(PRIMARY_KEY),
        AddressEntity(
        ...
        datetime.fromisoformat(ride_planning_dto.get("departure_datetime")),
        RidePlanningStatusEnum[ride_planning_dto.get("status")].value,
        datetime.fromisoformat(ride_planning_dto.get("created_at")),
        datetime.fromisoformat(ride_planning_dto.get("modified_at"))
    )
